...
#~~ Make subsetted structure file

# get genotypes

system("mkdir data/out/structure_sub/temp")
system("cut -d ' ' -f 7- data/out/plink/gl_sec_het_rhina_drop.ped > data/out/structure_sub/temp/A")

# convert values
system("sed 's/A/1/g' data/out/structure_sub/temp/A > data/out/structure_sub/temp/B")
system("sed 's/T/2/g' data/out/structure_sub/temp/B > data/out/structure_sub/temp/C")
system("sed 's/G/3/g' data/out/structure_sub/temp/C > data/out/structure_sub/temp/D")
system("sed 's/C/4/g' data/out/structure_sub/temp/D > data/out/structure_sub/temp/E")
system("sed 's/0/-9/g' data/out/structure_sub/temp/E > data/out/structure_sub/temp/F")

# get ids
system("awk '{print $2}' data/out/plink/gl_sec_het_rhina_drop.ped > data/out/structure_sub/temp/ids")

# make structure input file

gen <- fread("data/out/structure_sub/temp/F", header = F)
...
ids <- fread("data/out/structure_sub/temp/ids", header = F) %>%
  left_join(meta, by = c("V1" = "id")) %>%
  mutate(pop = gsub(" ", "_", pop)) %>%
  mutate(POP = case_when(pop == "oman" ~ 1,
                         pop == "saudi_arabia" ~ 2,
...
write.table(ids, "data/out/structure_sub/temp/ids_pop.txt", col.names = F, row.names = F, quote = F)
system("paste -d ' ' data/out/structure_sub/temp/ids_pop.txt data/out/structure_sub/temp/F > data/out/structure_sub/temp/rhina_handmade_rad_drop.stru")

# sort structure files by population

system("sort -k 2 data/out/structure_sub/temp/rhina_handmade_rad_drop.stru > data/out/structure_sub/rhina_handmade_rad_drop_sort.stru")
system("rm -r data/out/structure_sub/temp")

scp data/out/structure_sub/rhina_handmade_rad_drop_sort.stru ehumble@eddie.ecdf.ed.ac.uk:/exports/cmvm/eddie/eb/groups/ogden_grp/structure/data/raw/rhina_handmade_rad_drop_sort.stru
